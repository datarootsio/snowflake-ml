...
import toml
from layout_utils import footer, header
from snowflake_helpers import convert_df_types, init_connection, snowflake2pd
...
def _filter_options(
    df: pd.DataFrame, date_col: str = "CREATED_DATE"
    ...
    selected = {
        k: st.multiselect(k, options=v, default=v.tolist()) for k, v in unq_vals.items()
    }
    # Date filters
    _ds = _df[date_col]
    dates = st.slider(
        "Date", min_value=_ds.min(), max_value=_ds.max(), value=(_ds.min(), _ds.max())
    )
...
with col1:
    selected_dims, dates = _filter_options(df)
with col2:
    st.bar_chart(
        df.loc[
            reduce(
